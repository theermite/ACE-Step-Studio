  ...
  const refreshSongsList = useCallback(async () => {
    ...
    try {
      ...
      const loadedSongs: Song[] = response.songs.map(s => ({
        ...
        userId: s.user_id,
        creator: s.creator,
        ditModel: (s as any).dit_model || s.ditModel,
        lmModel: (s as any).lm_model || s.lmModel,
        lmBackend: (s as any).lm_backend || s.lmBackend,
        generationTime: (s as any).generation_time || s.generationTime,
        lrcContent: (s as any).lrc_content || s.lrcContent,
        openrouterModel: (s as any).openrouter_model || s.openrouterModel,
        bpm: s.bpm || (s as any).bpm || 0,
        keyScale: s.key_scale || (s as any).keyScale || '',
        timeSignature: s.time_signature || (s as any).timeSignature || '',
        generationParams: (() => {
